...
export default function PurchaseCalculator() {
  ...
  // Tooltips
  const PAYMENT_BREAKDOWN_TOOLTIP =
  "Payment Breakdown: A breakdown of your total payment so you can see where the money is allocated.";
...
<div className="grid grid-cols-[1fr_430px] gap-x-4 gap-y-3">
  {/* Row 1: KPI strips INLINE (left + right) */}
      ...
      <CardHeader className="pc-card__header">
        ...
        <UiTooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      aria-label="Early Payoff Strategy: Add an extra payment and see how many months you can eliminate on the back end of the loan."
                      className="inline-flex items-center ml-1 text-[#0F2C4C] focus:outline-none"
                    >
                      <Info className="h-4 w-4"/>
                    </button>
      ...
      <CardContent className="pc-card__body">
        <div className="space-y-4">
          <div>
            ...
            <Input type="text" inputMode="decimal" value={extraMonthly} onChange={(e) => setExtraMonthly(Number(e.target.value || 0))} className="eps-input" />
          </div>

          <div>
            <div className="text-sm font-medium mb-2">Increase Frequency</div>
            <div className="flex gap-3">
              <Button className={buttonVariants({ variant: increaseFreq === "monthly" ? "default" : "secondary" })} onClick={() => setIncreaseFreq("monthly")}>Monthly</Button>
              <Button className={buttonVariants({ variant: increaseFreq === "biweekly" ? "default" : "secondary" })} onClick={() => setIncreaseFreq("biweekly")}>Bi weekly</Button>
              <Button className={buttonVariants({ variant: increaseFreq === "weekly" ? "default" : "secondary" })} onClick={() => setIncreaseFreq("weekly")}>Weekly</Button>
            </div>
          </div>
        </div>
      </CardContent>
      ...
      <CardHeader className="pc-card__header">
        ...
        <UiTooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              aria-label="Lump Sum Payment: Shorten your loan term by paying a lump sum all to principal."
              className="inline-flex items-center ml-1 text-[#0F2C4C] focus:outline-none"
            >
              <Info className="h-4 w-4"/>
            </button>
      ...
      <CardContent className="pc-card__body">
        <div className="space-y-4">
          <div>
            ...
            <Input type="text" inputMode="decimal" value={lumpSum} onChange={(e) => setLumpSum(Number(e.target.value || 0))} className="eps-input" />
          </div>

          <div>
            <div className="text-sm font-medium mb-2">Frequency</div>
            <div className="flex gap-3">
              <Button className={buttonVariants({ variant: lumpFreq === "once" ? "default" : "secondary" })} onClick={() => setLumpFreq("once")}>One time</Button>
              <Button className={buttonVariants({ variant: lumpFreq === "yearly" ? "default" : "secondary" })} onClick={() => setLumpFreq("yearly")}>Yearly</Button>
              <Button className={buttonVariants({ variant: lumpFreq === "quarterly" ? "default" : "secondary" })} onClick={() => setLumpFreq("quarterly")}>Quarterly</Button>
            </div>
          </div>
        </div>
      </CardContent>
